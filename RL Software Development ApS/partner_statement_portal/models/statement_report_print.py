...
class StatementReportWizardInherit(models.Model):
    ...
    _inherit = ['rlbooks_statement.report.print','portal.mixin']
        
    def _compute_access_url(self):

        super(StatementReportWizardInherit, self)._compute_access_url()
        for order in self:
            order.access_url = '/my/statements/%s' % (order.id)

    def _get_portal_return_action(self):
        """ Return the action used to display statements when returning from customer portal. """
        self.ensure_one()
        return self.env.ref('partner_statement_portal.action_statements_with_onboarding')
