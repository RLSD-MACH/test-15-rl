...
class PurchaseOrderInherit(models.Model):
    ...
    fsc_certificate_id = fields.Many2one('fsc.certificate', string='FSC Certificate', required=False, domain="[('partner_id','=',partner_id),'|', ('company_id', '=', False), ('company_id', '=', company_id)]")
            ...
            for record in self:
                ...
                if record.state not in ['draft', 'sent']:
                    ...
                    if 'fsc_certificate_id' in vals or 'fsc_claim_id' in vals:
                        ...
                        raise UserError("You can't change FSC-settings on a PO with state: %s | vals: %s" % (str(record.state),str(vals)))
        ...
        if record.state not in skip_states or create == True:
            ...
            if 'order_line' in vals:
                ...
                if len(claims) == 1:
                    ...
                    vals.update(fsc_claim_id = claims[0].id )
                    ...
                    vals.update(fsc_claim_id = False)
