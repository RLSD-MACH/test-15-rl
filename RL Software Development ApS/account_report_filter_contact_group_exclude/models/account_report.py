...
class AccountReportInherit(models.AbstractModel):
    ...
    filter_contact_groups_exclude = None
    ...
    def _init_filter_partner(self, options, previous_options=None):
        ...
        if not self.filter_contact_groups_exclude:           
            return

        previous_contact_groups = (previous_options or {}).get('contact_groups_exclude', [])  
        
        if str(previous_contact_groups).startswith("res.partner.group(") == True:
            
            selected_contact_groups = None
            options['selected_contact_groups_names_exclude'] = None
            ...
            options['selected_contact_groups_names_exclude'] = values
                                  
        options['contact_groups_exclude'] = self._get_filter_contact_groups_exclude(selected_contact_groups, not_set)

    def _get_filter_contact_groups_exclude(self, selected_contact_groups, not_set=False):
    ...
    def _get_options_partner_domain(self, options):
        ...
        if options.get('contact_groups_exclude'):  
          
            account_position_ids = [int(acc.get('id') if acc.get('id') != 'zero' else False) for acc in options['contact_groups_exclude'] if acc.get('selected') == True]

            if len(account_position_ids) > 0:

                domain.append(('partner_id.group_id', 'not in', account_position_ids))
...
class ReportAccountAgedPartnerInherit(models.AbstractModel):
    ...
    filter_contact_groups_exclude = True
