...
class Container(models.Model):
    ...
    volume_cubic_meter = fields.Float(related='container_size_id.capacity_cubic_meter', string="Capacity Cubic Meter")
    ...
    @api.depends('volume_cubic_meter_total', 'capacity_cubic_meter')
    def _compute_remaining_capacity_cubic_meter(self):
        
        for record in self:
                    
            record.remaining_capacity_cubic_meter = record.capacity_cubic_meter - record.volume_cubic_meter_total

    @api.depends('weight_kg_total', 'capacity_kg')
    def _compute_remaining_capacity_kg(self):
        
        for record in self:
                    
            record.remaining_capacity_kg = record.capacity_kg - record.weight_kg_total
