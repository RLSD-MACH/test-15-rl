...
class StockMoveInherit(models.Model):
    ...
    def _default_owner_id(self):
        if self.env.context.get('default_owner_id'):
            return self.env.context.get('default_owner_id')
        return False

    # container_id = fields.Many2one('container', string='Container')
    owner_id = fields.Many2one('res.partner', string='From Owner', ondelete='restrict', default=_default_owner_id)
    # sale_order_id = fields.Many2one('sale.order', string='Sale order', requied=False)
    ...
    def _action_assign(self):
        ...
        def _get_available_move_lines(move):
            ...
            available_move_lines_new = {}
            
            for (location_id, lot_id, package_id, owner_id), quantity in available_move_lines.items():
                 if quantity > 0:
                   available_move_lines_new[(location_id, lot_id, package_id, owner_id)] = quantity
            ...
            if move._should_bypass_reservation():
                ...
                if missing_reserved_quantity and move.product_id.tracking == 'serial' and (move.picking_type_id.use_create_lots or move.picking_type_id.use_existing_lots):
                    ...
                    to_update = move.move_line_ids.filtered(lambda ml: ml.product_uom_id == move.product_uom and
                                                            ...
                                                            not ml.owner_id
                                                            # ml.owner_id == move.owner_id 
                                                            # and ml.container_id == move.container_id
                                                            )
                    if to_update:
                ...
                if float_is_zero(move.product_uom_qty, precision_rounding=move.product_uom.rounding):
                    ...
                    available_quantity = move._get_available_quantity(move.location_id, package_id=forced_package_id, owner_id=move.owner_id)
                    if available_quantity <= 0:
                        continue
                    taken_quantity = move._update_reserved_quantity(need, available_quantity, move.location_id, package_id=forced_package_id, owner_id=move.owner_id, strict=False)
