...
class MailMailInherit(models.Model):
    ...
    email_bcc = fields.Char('BCC', help='BCC copy message recipients')

    recipient_cc_ids = fields.Many2many('res.partner', relation="mail_mail_recipient_cc_ids_res_partner_rel", string='To (Partners CC)',
        context={'active_test': False})

    recipient_bcc_ids = fields.Many2many('res.partner', relation="mail_mail_recipient_bcc_ids_res_partner_rel", string='To (Partners BCC)',
        context={'active_test': False})
    ...
    def _send(self, auto_commit=False, raise_exception=False, smtp_session=None):
        ...
        for mail_id in self.ids:
            ...
            try:
                ...
                for partner in mail.recipient_cc_ids:
                ...
                if mail.recipient_ids:
                    ...
                    values = mail._send_prepare_values(partner=mail.recipient_ids[0])          
                
                elif mail.recipient_cc_ids:

                    values = mail._send_prepare_values(partner=mail.recipient_cc_ids[0])    
                
                elif mail.recipient_bcc_ids:

                    values = mail._send_prepare_values(partner=mail.recipient_bcc_ids[0])  
                ...
                values['email_bcc'] = email_bcc

                email_list.append(values)
                raise UserError(str(len(email_list)) + " - cc " + str(tools.email_split(email_list[0].get('email_cc'))) + " bcc " + str(tools.email_split(email_list[0].get('email_bcc'))))
                # headers
                headers = {}
                ...
                for email in email_list:
                    msg = IrMailServer.build_email(
                        ...
                        email_to=tools.email_split(email.get('email_to')),
                        subject=mail.subject,
                        body=email.get('body'),
                        body_alternative=email.get('body_alternative'),
                        email_cc=tools.email_split(email.get('email_cc')),
                        email_bcc=tools.email_split(email.get('email_bcc')),
                    ...
                    try:
                        ...
                        if str(error) == IrMailServer.NO_VALID_RECIPIENT:
                            ...
                            _logger.info("Ignoring invalid recipients for mail.mail %s: %s",
                                         mail.message_id, email.get('email_to') + " - cc: " + email.get('email_cc') + " - bcc: " + email.get('email_bcc'))
                        else:
                            raise
